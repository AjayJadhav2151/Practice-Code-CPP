...
int main()
{
	...
	//cout << "Display Player having Maximum runs" << endl;
		int Maxrun = 0;
	for (int i = 0; i < 10; i++)
	{
		if (p1[i].getrun() > p1[Maxrun].getrun())
		{
			Maxrun = i;
		}
	}
	...
	/*cout << "Display Player having Maximum Wicket" << endl;*/
	int Maxwic = 0;
	for (int i = 0; i < 10; i++)
	{
		if (p1[i].getwic() > p1[Maxwic].getwic())
		{
			Maxwic = i;
		}
	}
